...
def plot_combined(mc_values, gbs_values, gt_values, filename):
    ...
    axes[0].set_xlabel(r"${V^{\text{MC}}_{\text{Haf}^2}}/{V^{\text{GBS-I}}_{\text{Haf}^2}}$", fontsize=20)
    axes[0].set_ylabel('Frequency', fontsize=20)
    axes[0].set_title(f'When GBS-I is more efficient', fontsize=20)

    # Plot 3: Histogram of GBS/MC (≤ 1, inverted to ≥ 1)
    axes[1].hist(ratio_filtered2, edgecolor='black', alpha=0.75)
    axes[1].set_xlabel(r"${V^{\text{GBS-I}}_{\text{Haf}^2}}/{V^{\text{MC}}_{\text{Haf}^2}}$", fontsize=20)
    ...
    axes[2].set_xticklabels(['GBS-I gains', 'MC gains'])
...
def track_results_in_table():
    ...
    for i, N in enumerate(N_values):
        ...
        for j, K in enumerate(K_values):
            ...
            path_to_folder = f'/work/GBSPE/exp/hafsq/{exp_id}'
            ...
            try:
                ...
                filename = f'/work/GBSPE/exp/hafsq/fig2/{exp_id}.png'
